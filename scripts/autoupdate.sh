...
# truncate / ddnz 辅助工具来源，可通过 TOOL_REPO 覆盖
TOOL_REPO="${TOOL_REPO:-stupidloud/nanopi-openwrt}"
wget -NP /tmp https://mirror.ghproxy.com/https://raw.githubusercontent.com/$TOOL_REPO/zstd-bin/truncate
wget -NP /tmp https://mirror.ghproxy.com/https://raw.githubusercontent.com/$TOOL_REPO/zstd-bin/ddnz
...
# 固件来源仓库，可通过环境变量 REPO 覆盖，默认使用当前项目仓库
REPO="${REPO:-iceend/nanopi-openwrt}"
set +e
wget https://mirror.ghproxy.com/https://github.com/$REPO/releases/download/$(date +%Y-%m-%d)/$board_id$ver.img.gz -O- | gzip -dc > $board_id.img
if [ $? -eq 0 ]; then
	wget https://mirror.ghproxy.com/https://github.com/$REPO/releases/download/$(date +%Y-%m-%d)/$board_id$ver.img.md5 -O md5sum.txt
	echo -e '\e[92m今天固件已下载，准备解压\e[0m'
else
	echo -e '\e[91m今天的固件还没更新，尝试下载昨天的固件\e[0m'
	wget https://mirror.ghproxy.com/https://github.com/$REPO/releases/download/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.gz -O- | gzip -dc > $board_id.img
	if [ $? -eq 0 ]; then
		wget https://mirror.ghproxy.com/https://github.com/$REPO/releases/download/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.md5 -O md5sum.txt
