function merge_package(){
    ...
    if git clone --depth=1 --single-branch $1; then
        ...
        echo "WARNING: Failed to clone $1, skipping $pkg"
    fi
    rm -rf $repo
}
...
rm -rf package/feeds/nas_luci/luci-app-ddnsto
...
merge_package "-b dev https://github.com/vernesong/OpenClash" OpenClash/luci-app-openclash
merge_package https://github.com/NateLol/luci-app-oled luci-app-oled
# brook has been removed from openwrt-passwall-packages upstream; keep this disabled to avoid mv failing on a missing path in GitHub Actions.
...
# trojan-go has been removed from openwrt-passwall-packages upstream; keep this disabled to avoid mv failing on a missing path in GitHub Actions.
...
merge_package "-b main https://github.com/Openwrt-Passwall/openwrt-passwall" openwrt-passwall
